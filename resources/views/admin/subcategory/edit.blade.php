          ...
          <div class="card card-primary">
              <div class="card-header">
                <div class="row">
                  ...
                  <div class="col-md-6">
                    <a class="float-right" href="{{url('/admin/subcategory/create/step2')}}?id={{$data->id}}">Upload Multiple Images</a>
                  </div>
                </div>
              </div>
              <!-- /.card-header -->
                ...
                <div class="card-body">
                  <div class="form-group {{ $errors->has('name') ? ' has-error' : '' }}">
                    ...
                    <select class="form-control" id="parent_id">
                      ...
                      <option @if($category->id == $data->parent_id) selected @endif value="{{$category->id}}">{{$category->name}}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="form-group {{ $errors->has('name') ? ' has-error' : '' }}">
                    <label for="name">Subcategory Name <span class="required">*</span></label>
                    <input name="name" type="text" class="form-control" id="name" placeholder="Enter Subcategory Name" value="{{$data->name}}">
                    <span style="font-size: 17px;" id="required_name"></span>
                  </div>
                  <div class="row">
                    <div class="col-md-4">
                      <label for="image">Subcategory Icon <span class="required">*</span></label>
                      <img src="{{ URL::to('/') }}/uploads/category/{{ @$data->image }}" alt="" class="img-responsive" style="width: 100%;"/>
                  ...
                  <div class="form-group">
                    <div class="form-group">
                        <div class="custom-control custom-radio">
                          <input class="custom-control-input is_normal" type="radio" id="customRadio1" name="is_normal" value="yes" @if(@$data->is_normal == "yes") checked @endif>
                          <label for="customRadio1" class="custom-control-label">Normal</label>
                        </div>
                        <div class="custom-control custom-radio">
                          <input class="custom-control-input is_normal" type="radio" id="customRadio2" name="is_normal" value="no" @if(@$data->is_normal == "no") checked @endif>
...
$('.upload-image').on('click', function (ev) {
  ...
  if (requiredCheck == 1) {
    resize.croppie('result', {
      ...
      var is_normal = $('input[name="is_normal"]:checked').val();
      var id = $('#id').val();
      $.ajax({
        headers: {
        ...
        type: "POST",
        data: {"image":img,"name":name,"has_image":has_image,"parent_id":parent_id,"id":id,"meta_title":meta_title,"meta_desc":meta_desc,"is_normal":is_normal},
