              ...
              <div class="card-body">
                <table class="table table-bordered">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>
                        <a href="{{ url('/godown/order-details')}}?order_id={{$row->order_id}}">{{ @$row->order_id}}</a>
                        <span>{{ @$username->name}}</span><br />
                          <span>{{ @$username->phone}}</span>
                      </td>
                      <td>
                        <span>&#8377;{{ @$row->order_price}}.00</span><br>
                      ...
                      <td>
                        @if($row->status == "cancelled" || $row->status == "delivered")
                          {{$row->status}}
                        @else
                          @if($row->status == "order_preparing") Order Preparing @endif @if($row->status == "ontheway") Order Ontheway @endif @if($row->status == "delivered") Order Delivered @endif @if($row->status == "order_received") Order Received @endif @if($row->status == "order_confirmed") Order Accepted @endif @if($row->status == "cancelled") Order Rejected @endif<button type="button" class="btn btn-info" data-toggle="modal" data-target="#modal-status{{ @$row->id}}">
                          Change Status
                          </button>
                        @endif
                      </td>
                      ...
                      <div class="modal-dialog modal-sm">
                        <div class="modal-content">
                          ...
                          <div class="modal-body">
                            <div class="col-sm-12">
                              <form method="post" action="{{url('/godown/assignDelivery')}}">
                                @csrf
                    ...
                    <div class="modal fade" id="modal-status{{ @$row->id}}">
                        <div class="modal-dialog modal-sm">
                          <div class="modal-content">
                            ...
                            <div class="modal-body">
                              <div class="col-sm-12">
                                <form method="post" action="{{url('/godown/changeOrderStatus')}}">
                                  @csrf
                                  <div class="form-group">
